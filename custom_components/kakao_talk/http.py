...
class KakaoConfig:
    def __init__(self, hass, config):
        ...
        hass.async_create_task(self._load_session(hass))

    async def _load_session(self, hass):
        _filepath = "{}/{}".format(hass.config.config_dir, SESSION_FILE)
        if os.path.exists(_filepath):
            async with aiofiles.open(_filepath, "r") as file:
                if os.stat(_filepath).st_size == 0:
                    _LOGGER.warning("Please login on Kakaotalk!!")
                else:
                    self._refresh_token = await file.read()
                    _LOGGER.debug("KAKAO SESSION {}".format(self._refresh_token))

    async def save_refresh_token(self):
        _filepath = "{}/{}".format(self.hass.config.config_dir, SESSION_FILE)
        try:
            async with aiofiles.open(_filepath, "w") as file:
                await file.write(self._refresh_token)
        except Exception as exception:
            _LOGGER.error(f"Error saving refresh token: {exception}")
    ...
    async def get_setup_page(self):
        ...
        _filecontent = ""
        try:
            async with aiofiles.open(_path, mode="r", encoding="utf-8", errors="ignore") as localfile:
                _filecontent = await localfile.read()
        except Exception as exception:
            _LOGGER.error(f"Error reading setup.html: {exception}")
